...
- Zero false positives — exact same input always returns exact same output
...
- TTL: configurable, default 15 minutes (shorter than exact — semantic similarity is approximate)

### Layer 3 — Workflow Cache
...
|---------|---------|--------------|
| In-memory dict | Development, single-process | Default (no config) |
| Redis | Production, multi-instance | `REDIS_URL=redis://localhost:6379/0` |

The interface is identical — swapping from in-memory to Redis requires only setting `REDIS_URL` in `.env`. No code changes.

---

## Configuration (Phase 4 — `config/policies.yaml`)

```yaml
cache:
  enabled: true
  exact_ttl_seconds: 3600        # 1 hour
  semantic_ttl_seconds: 900      # 15 minutes
...
Cache keys always include `team_id`. A cached response from `team-engineering` is never returned to `team-finance`, even for identical prompts. Confidential-privacy tasks (`privacy: "confidential"`) are never cached.

---

## Cache Hit Rate Visibility (Phase 5 Dashboard)

The Phase 5 dashboard will include a cache hit rate panel showing:
- Exact cache hits per hour/day
- Semantic cache hits per hour/day
- Estimated cost saved by caching (in USD)
...
- Agent workflows with retry logic (workflow cache eliminates re-runs)
