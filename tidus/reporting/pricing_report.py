...
class PricingReport:
    ...
    # Drift alarm — when the active revision hasn't moved in N+ days AND a
    # live pricing source ran successfully this cycle, the underlying market
...
class PricingReportGenerator:
    ...
    def _render_markdown(
        ...
        # Drift alarm — comes first so it can't be missed in the email preview.
        if report.drift_alarm_days is not None:
            lines += [
                f"> 🚨 **Drift alarm:** the active revision hasn't changed in "
                f"**{report.drift_alarm_days}+ days** even though live pricing sources "
                f"ran successfully. The market is moving — our sources probably aren't "
                f"catching it. Check `OpenRouterPricingSource` health and the "
                f"`hardcoded_source.py` baseline against vendor pricing pages.",
                "",
            ]
        ...
        lines += [
            ...
            "*Source data: HardcodedSource (verified-by-hand anchor, confidence 0.7) + "
            "OpenRouterPricingSource (live multi-vendor feed, confidence 0.75) + "
            "TidusPricingFeedSource (confidence 0.85, if configured).*  ",
            "*Consensus uses MAD outlier detection; freshness breaks ties — so the "
            "live OpenRouter quote wins over a stale hardcoded entry on every match.*",
        ]
